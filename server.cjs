...
function recordRequestRemote({ model, inputTokens, outputTokens, tokenIdx, success, ms }) {
  ...
  supabase.from('hermes_requests')
    ...
    .catch((error) => LOG.err(`Supabase insert requests gagal: ${error.message}`));
}
...
function saveLocalMemory(sessionKey, turns, summary) {
  supabase.from('hermes_sessions')
    ...
    .catch((err) => LOG.err(`Supabase upsert memory crash: ${err.message}`));
  ...
  supabase.from('hermes_sessions')
    ...
    .catch((err) => LOG.warn(`Auto-purge database error: ${err.message}`));
}
...
function archiveSummary(sessionKey, oldSummary) {
  ...
  supabase.from('hermes_memory_archive')
    ...
    .catch((err) => LOG.err(`Supabase archive crash: ${err.message}`));
}
...
function setSessionIndexRemote(key, index) {
  supabase.from('hermes_sessions')
    ...
    .catch((err) => LOG.err(`Supabase session save crash: ${err.message}`));
}
...
function callGemini(payload, apiKey, tokenIndex, model, reqId, timeoutMs) {
    ...
    const req = https.request(options, (res) => {
      ...
      res.on('end', () => {
        ...
        try {
          ...
          if (!hasContent) return reject({ type: 'EMPTY_CONTENT', message: 'No content candidates', index: tokenIndex });
          ...
          if (!hasRealContent) return reject({ type: 'EMPTY_CONTENT', message: 'Parts exist but all empty', index: tokenIndex });
...
async function tryModelWithPool(body, model, startIndex, reqId, timeoutMs, sessionKey) {
  // CLONE BODY mencegah mutasi history message di round/offset berikutnya
  const clonedBody = JSON.parse(JSON.stringify(body));
  const payload = toGeminiPayload(clonedBody, reqId, body._memoryInjection, sessionKey);
  ...
  for (let round = 0; round < MAX_ROUNDS; round++) {
    for (let offset = 0; offset < total; offset++) {
      ...
      try {
        const result = await callGemini(payload, TOKEN_POOL[idx], idx, model, reqId, timeoutMs);
        return { res: result, idx };
      } catch (err) {
        LOG.warn(`[${reqId}] tok#${idx} failed: ${err?.type} → next`);
        if (/thought_signature/i.test(err?.message || '')) return null;
        if (err?.type === 'API_ERROR' && /not found|invalid|does not exist|unsupported/i.test(err?.message || '')) return null;
        continue;
      }
    }
...
function toOpenRouterPayload(body, model, memoryInjection, sessionKey, reqId) {
  ...
  const payload = { model: model, messages: finalMessages, stream: false };
  if (body.max_tokens) payload.max_tokens = Math.min(body.max_tokens, 4096);
...
function callOpenRouter(body, model, reqId, timeoutMs, sessionKey) {
  return new Promise((resolve, reject) => {
    ...
    const req = https.request(options, (res) => {
      ...
      res.on('end', () => {
        ...
        if (res.statusCode !== 200) {
          LOG.warn(`[${reqId}] OpenRouter HTTP Error: ${res.statusCode}`);
          return reject({ type: 'API_ERROR', message: `HTTP Status ${res.statusCode}` });
        }
        try {
          const json = JSON.parse(raw);
          if (json.error) return reject({ type: 'API_ERROR', message: json.error.message });
          ...
          if (msg.tool_calls?.length) {
            LOG.tool(`[${reqId}] OpenRouter Tools Triggered: [${msg.tool_calls.map(t => t.function?.name).join(', ')}]`);
            msg.tool_calls.forEach(tc => {
              let parsedArgs = {};
              try { parsedArgs = typeof tc.function.arguments === 'string' ? JSON.parse(tc.function.arguments) : tc.function.arguments; } catch (e) { parsedArgs = { raw: tc.function.arguments }; }
              pseudoGemini.candidates[0].content.parts.push({ functionCall: { name: tc.function.name, args: parsedArgs } });
            });
          } else if (msg.content) {
    ...
    req.on('error', e => { if (!done) reject({ type: 'NETWORK_ERROR', message: e.message }); });
    req.write(payload); req.end();
  });
}
  ...
  if (isOpenRouterSession) {
    ...
    for (let i = 0; i < chain.length; i++) {
      ...
      try {
        ...
        LOG.warn(`[${reqId}] OpenRouter "${model}" failed (${err?.type || 'ERROR'}) → next`);
      }
    }
    LOG.fallback(`[${reqId}] Semua OpenRouter exhausted → Last resort Gemini Pool`);
  }
...
function buildOpenAIResponse(geminiRes, chunkId, model, stream, res, reqId) {
  ...
  if (firstPart?.functionCall) {
    const toolCalls = parts.filter(p => p.functionCall).map((p, i) => {
      const cleanArgs = JSON.parse(JSON.stringify(p.functionCall.args || {}, (k, v) => typeof v === 'string' ? stripResponseTags(v) : v));
      return { id: `call_${chunkId}_${i}`, type: 'function', function: { name: p.functionCall.name, arguments: JSON.stringify(cleanArgs) } };
    });
    ...
    if (stream) {
      if (!res.headersSent) { res.setHeader('Content-Type', 'text/event-stream'); res.setHeader('Cache-Control', 'no-cache'); }
  ...
  let text = (responseMatch && responseMatch[1]) ? responseMatch[1] : rawText;
  const compact = (compactMatch && compactMatch[1]) ? compactMatch[1].trim() : null;
  ...
  LOG.out(`[${reqId}] → TEXT: ${text.length} chars`);

  if (stream) {
    if (!res.headersSent) { res.setHeader('Content-Type', 'text/event-stream'); res.setHeader('Cache-Control', 'no-cache'); res.setHeader('Connection', 'keep-alive'); }
...
async function handleChat(req, res) {
  ...
  body._t0 = Date.now();
  LOG.req(`[${reqId}] POST /chat | model=${model} msgs=${messages.length} stream=${stream}`);
  ...
  const hasToolChain = messages.some(m => m.role === 'tool' || m.role === 'function' || (m.role === 'assistant' && m.tool_calls?.length > 0));
  const hasTools = body.tools && body.tools.length > 0;
  
  // Perbaikan Timeout Dinamis berbasis ketersediaan deskripsi tools berkas
  const timeoutMs = (hasToolChain || hasTools) ? TIMEOUT_TOOL : TIMEOUT_NORMAL;

  if (!hasToolChain && !hasTools && (maxTokens <= 30 || lastContent.includes('title') || lastContent.includes('judul') || /generate a short.*title/i.test(sysContent))) {
  ...
  const isCronjob = /\[IMPORTANT:.*cron job/i.test(firstUserMsg) || /cron job.*failed/i.test(firstUserMsg);
  
  const sessionKey = isCronjob ? `cron_${hashKey(firstUserMsg.slice(0, 100))}`
    : /expert ai programming|vs code|github copilot|cline|roo clinic/i.test(sysContent) ? 'copilot'
    : /asisten pribadi|sebastian/i.test(sysContent) ? 'sebastian' : hashKey(sysContent.slice(0, 200));
  ...
  try {
    ...
    if (heartbeat) clearInterval(heartbeat); // Bersihkan sukses interval
    setSessionIndexRemote(sessionKey, winnerIdx);
    globalIndex = (winnerIdx + 1) % TOKEN_POOL.length;

    if (!isCronjob) {
      const rawAssistantText = geminiRes.candidates?.[0]?.content?.parts?.filter(p => p.text).map(p => p.text).join('') || '';
      ...
      const compact = res._inlineCompact || null;

      const newTurns = [];
      if (lastUserMsg) newTurns.push({ role: 'user', content: lastUserMsg.length > 200 ? lastUserMsg.slice(0, 200) + '...' : lastUserMsg, ts: Date.now() });
      if (assistantText || compact) newTurns.push({ role: 'assistant', content: compact || assistantText, ts: Date.now() });
      ...
      if (combinedTurns.length >= MEMORY_CONFIG.summary_threshold) {
        const summaryPrompt = `Ringkasan sebelumnya: ${sessionSummary || '(belum ada)'}\nPercakapan baru:\n${combinedTurns.map(t => `${t.role}: ${t.content}`).join('\n')}\n\nBuat SATU ringkasan baru gabungan maksimal 2-3 kalimat.`;
        try {
          const summaryBody = { messages: [{ role: 'user', content: summaryPrompt }], _memoryInjection: '' };
          const { res: sumRes } = await tryModelWithPool(summaryBody, DEFAULT_MODEL, globalIndex, reqId + '_sum', TIMEOUT_NORMAL, sessionKey);
          const sumRaw = sumRes?.candidates?.[0]?.content?.parts?.map(p => p.text || '').join('').trim() || '';
          ...
          LOG.warn(`[${reqId}] Summary builder skip: ${e.message}`);
        }
      }
    ...
    if (heartbeat) clearInterval(heartbeat); // FIX: Mencegah Memory Leak fatal jika API OpenRouter/Gemini crash
...
app.use((req, res, next) => { if (!LOG_SUPPRESS.has(`${req.method}:${req.path}`)) LOG.req(`→ [${req.method}] ${req.url}`); next(); });
...
const MODELS_LIST = {
  object: 'list',
  data: [...MODEL_FALLBACK_CHAIN, ...OPENROUTER_COPILOT_CHAIN, ...OPENROUTER_WHATSAPP_CHAIN].map(id => ({ id, object: 'model', created: 1700000000, owned_by: 'google' }))
};
...
app.get('/health', (req, res) => {
  ...
  res.status(200).json({
    status: 'OK',
    uptime: `${((Date.now() - healthStats.startTime.getTime()) / 1000).toFixed(2)}s`,
    requests: { total: healthStats.totalRequests, success: healthStats.successfulRequests, fail: healthStats.failedRequests },
...
app.listen(PORT, () => {
  ...
  LOG.boot(`Pool: ${TOKEN_POOL.length} tokens terdeteksi.`);
});